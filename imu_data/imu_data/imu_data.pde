...
void setup()
{
  ...
  table.addColumn("imu-roll");
  table.addColumn("imu-pitch");
  table.addColumn("imu-yaw");
...
void serialEvent(Serial myPort){
  ...
  if (val!= null) { //We have a reading! Record it.
    ...
    float angles[] = float(split(val, ',')); //parses the packet from Arduino and places the valeus into the sensorVals array. I am assuming floats. Change the data type to match the datatype coming from Arduino. 
    ...
    newRow.setFloat("imu-roll", angles[0]);
    newRow.setFloat("imu-pitch", angles[1]);
    newRow.setFloat("imu-yaw", angles[1]);
